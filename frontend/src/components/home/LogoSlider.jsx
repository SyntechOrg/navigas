      ...
      try {
        ...
        if (data?.data) {
          const formattedData = data.data.map((item) => {
            ...
            const imageField = attrs?.bild || attrs?.Bild || attrs?.image || attrs?.Image;
            ...
            if (imageField) {
                ...
                if (singleImage) {
                    ...
                    imageUrl = imgAttrs?.url || imgAttrs?.formats?.small?.url || imgAttrs?.formats?.thumbnail?.url;
                }
            }

            if (!imageUrl) {
               console.log("LogoSlider: No image found for", attrs?.modell, { imageField, attrs });
            }

            const rawPrice = attrs?.preis || attrs?.Preis || attrs?.price || attrs?.Price;
            const formattedPrice = rawPrice 
              ? (String(rawPrice).match(/^\d+$/) ? `Ab ${rawPrice} CHF/Monat` : rawPrice)
              : "";
